...
def _import_plotting_backend(cache_dir: Path):
...
def _validate_simulation_sources(manifest: Dict[str, object]) -> None:
    ...
    critical_names = ["synthetic_core.py"]
    mismatches = []
    ...
    if mismatches:
        ...
        raise ValueError(
            "Cached simulation results do not match the current numerical simulation code "
            f"for: {joined}. Please rerun synthetic_benchmark.py before plotting."
        )
...
def _write_trajectory_figure(items: List[Dict[str, object]], plots_dir: Path, plt) -> None:
    ...
    def _plot_indices(n_rounds: int, max_points: int = 2000) -> np.ndarray:
        if n_rounds <= max_points:
            return np.arange(n_rounds, dtype=int)
        idx = np.unique(np.linspace(0, n_rounds - 1, max_points, dtype=int))
        if idx[-1] != n_rounds - 1:
            idx = np.append(idx, n_rounds - 1)
        return idx
    ...
    for row_idx, item in enumerate(items):
        ...
        plot_idx = _plot_indices(rounds.size)
        plot_rounds = rounds[plot_idx]

        ax_regret = axes[row_idx, 0]
        regret_p = np.asarray(item["pareto_ucb1_regret_mean"], dtype=float)[plot_idx]
        regret_p_se = np.asarray(item["pareto_ucb1_regret_se"], dtype=float)[plot_idx]
        regret_w = np.asarray(item["width_guided_regret_mean"], dtype=float)[plot_idx]
        regret_w_se = np.asarray(item["width_guided_regret_se"], dtype=float)[plot_idx]

        ax_regret.plot(plot_rounds, regret_p, color=colors["pareto"], linewidth=2.0, label="Pareto UCB1")
        ax_regret.fill_between(
            plot_rounds,
            regret_p - regret_p_se,
            regret_p + regret_p_se,
            color=colors["pareto"],
            alpha=0.15,
        )
        ax_regret.plot(plot_rounds, regret_w, color=colors["width"], linewidth=2.0, label="Width-guided")
        ax_regret.fill_between(
            plot_rounds,
            regret_w - regret_w_se,
            regret_w + regret_w_se,
            color=colors["width"],
            alpha=0.15,
        )
        ...
        cert_frac = np.asarray(item["width_certified_fraction"], dtype=float)[plot_idx]
        ax_cert.plot(plot_rounds, cert_frac, color=colors["width"], linewidth=2.2)
        if np.isfinite(median_cert):
            ax_cert.axvline(
                median_cert,
                color="#444444",
...
def main() -> None:
    ...
    plt, matplotlib_version = _import_plotting_backend(out_dir)
